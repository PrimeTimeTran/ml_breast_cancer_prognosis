...
def update_paths(set_type):
    ...
    for row in data_rows:
        update_folder_name(row, set_type)
...
def sort_rows(set_type):
    for _, file in enumerate(get_files(set_type)):
        df = pd.read_csv(file)
        df = df.sort_values(
            by='PatientID', key=lambda x: x.map(sort_by_patient_ids))
