...
from .helpers import get_image_laterality, get_window_center, get_window_width, script_dir
...
def view_image(set_type, df, plt, num):
    ...
    image_path = os.path.join(script_dir, f"../tmp/{set_type}/manifest-1617905855234/", view_series["descriptive_path"])
    image = dcmread_image(fp=image_path, view=view)
    patient_id = view_series['PatientID']
    filename = f'{patient_id}-{view}.png'
    output_path = os.path.join(script_dir, f'../tmp/imgs/{set_type}/{filename}')
...
# def evaluate(
#     labels_fp: FilePathOrBuffer,
#     boxes_fp: FilePathOrBuffer,
#     predictions_fp: FilePathOrBuffer,
# ) -> Dict[str, float]:
